...
def print_tree(root: Dir, depth: int) -> None:
    click.echo(('--' * depth) + ' ' + str(root))
    for child in root.children:
        if isinstance(child, Dir):
            print_tree(child, depth + 2)
        else:
            click.echo(('--' * (depth + 1)) + ' ' + str(child))
    return
...
def run():
    with open(os.getcwd() + '/year_2022/day07/input.txt') as f:
        ...
        total_file_sistem_size = 70000000
        needed_free_size = 30000000
        total_used_size = root_dir.size
        total_unused_size = total_file_sistem_size - total_used_size
        need_to_delete_size = needed_free_size - total_unused_size

        selected_sizes = [s for s in sizes if s >= need_to_delete_size]

        click.echo(sorted(selected_sizes)[0])
